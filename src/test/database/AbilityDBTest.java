...
public class AbilityDBTest {
    ...
    static void setUpAll() {
        // Cria a tabela no banco de dados em memória uma vez antes de todos os testes.
    ...
    void setUp() {
        ...
        testAbility1 = new Ability("Tackle", "A basic attack", PokeType.GRASS, 10.0, 0.0);
        testAbility2 = new Ability("Water Gun", "Shoots water", PokeType.WATER, 15.0, 0.0);
    }

    @AfterEach
    void tearDown() {
        // Limpa a tabela depois de cada teste para garantir que os testes sejam independentes.
    ...
    void testInsertAndGetAbility() {
        ...
        // Assert: Verifica se a habilidade retornada não é nula e se seus dados estão corretos.
        assertNotNull(retrievedAbility, "A habilidade buscada não deveria ser nula.");
        assertEquals("Tackle", retrievedAbility.getName(), "O nome da habilidade deve ser o mesmo.");
        assertEquals(10.0, retrievedAbility.getDamage(), "O dano da habilidade deve ser o mesmo.");
        assertEquals(PokeType.GRASS, retrievedAbility.getType(), "O tipo da habilidade deve ser o mesmo.");
    }
    ...
    void testGetAbility_WhenNotFound_ReturnsNull() {
        ...
        assertNull(retrievedAbility, "Buscar uma habilidade inexistente deve retornar nulo.");
    }
    ...
    void testUpdateAbility() {
        ...
        Ability updatedInfo = new Ability("Tackle", "A powerful physical attack", PokeType.GRASS, 25.0, 0.0);
        ...
        assertEquals("A powerful physical attack", retrievedAbility.getDescription(), "A descrição deveria ter sido atualizada.");
        assertEquals(25.0, retrievedAbility.getDamage(), "O dano deveria ter sido atualizado.");
    }
    ...
    void testDeleteAbility() {
        ...
        AbilityDB.insertAbility(testAbility1);
        assertNotNull(AbilityDB.getAbility("Tackle"), "A habilidade deve existir antes de ser deletada.");

        // Act: Deleta a habilidade.
        AbilityDB.deleteAbility("Tackle");

        // Assert: Verifica se a habilidade não existe mais.
        assertNull(AbilityDB.getAbility("Tackle"), "A habilidade não deveria mais ser encontrada após a deleção.");
    }
    ...
    void testGetAllAbilities_NoFilter() {
        ...
        assertEquals(2, allAbilities.size(), "A lista deveria conter 2 habilidades.");
        assertTrue(allAbilities.stream().anyMatch(a -> a.getName().equals("Tackle")), "A lista deveria conter a habilidade 'Tackle'.");
        assertTrue(allAbilities.stream().anyMatch(a -> a.getName().equals("Water Gun")), "A lista deveria conter a habilidade 'Water Gun'.");
    }
    ...
    void testGetAllAbilities_WithFilterByType() {
        ...
        List<Ability> filteredAbilities = AbilityDB.getAllAbilities(Optional.of(filter));

        // Assert: Verifica se apenas a habilidade correta foi retornada.
        assertEquals(1, filteredAbilities.size(), "A lista filtrada deveria conter apenas 1 habilidade.");
        assertEquals("Water Gun", filteredAbilities.get(0).getName(), "A habilidade na lista filtrada deveria ser 'Water Gun'.");
    }
}
